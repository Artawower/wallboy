...
- Providers: Unsplash, Wallhaven, Bing Daily, Generic URL
...
| =--provider=    | Use specific provider (bing/wallhaven/unsplash/local) |
...
[[https://www.bing.com][Bing]] provides a new high-quality wallpaper every day. No API key required!
...
Features:
- Automatically fetches Bing's daily wallpaper (UHD resolution)
- Access to the last 8 days of wallpapers
- No queries needed - ignores search queries and uses Bing's curated daily images
- Perfect for a "set and forget" wallpaper source

Example config using only Bing:
...
dirs = ["~/Pictures/Wallpapers/Dark"]
upload-dir = "~/Pictures/Wallpapers/Dark"
providers = ["bing"]  # only use bing
# No queries needed for bing!
#+end_src
...
#+begin_src bash
# Get wallpaper only from Bing
wallboy next --provider bing

# Get wallpaper only from Wallhaven
wallboy next --provider wallhaven

# Get wallpaper only from local directories
wallboy next --provider local

# Combine with theme
wallboy next --theme dark --provider bing
#+end_src
